...
var JobManager = module.exports = function JobManager(options){
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        var shareDiff = diff1 / headerBigNum.toNumber() * shareMultiplier * 1000;
        console.log("diff1: "+diff1+" headerBigNum.toNumber(): "+headerBigNum.toNumber()+" shareMultiplier: "+shareMultiplier+" difficulty: "+difficulty);

        var blockDiffAdjusted = job.difficulty * shareMultiplier * 1000;

        //Check if share is a block candidate (matched network difficulty)
        if (job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, nTime).toString('hex');
            console.log("Entrou na condição 1");
        }
        else {
        	console.log("Entrou na condição 2");
