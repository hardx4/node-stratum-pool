...
var pool = module.exports = function pool(options, authorizeFn){
                ...
                for (var i = 0; i < results.length; i++){
                    ...
                    if (result.error) {
                        emitErrorLog('rpc error with daemon instance ' +
                                ...
                                JSON.stringify(result.error) + 'HEX: ' +blockHex
                        );
                        return;
                    }
                    else if (result.response === 'rejected') {
                        emitErrorLog('Daemon instance ' + result.instance.index + ' rejected a supposedly valid block hex: '+blockHex);
                        return;
                    }
                }
                emitLog('Submitted Block using ' + rpcCommand + ' successfully to daemon instance(s) | results: '+ JSON.stringify(results));
                callback();
            }
        );
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            var diff_n = "proof-of-work";
            if(options.coin.reward === 'POS')
            diff_n = "proof-of-stake";
            ...
            if(options.coin.name === 'martexcoin' || options.coin.name === 'gaycoin' || options.coin.name === 'bzlcoin'){
            options.initStats = {
                connections: rpcResults.getinfo.connections,
                difficulty: rpcResults.getinfo.difficulty[diff_n] * algos[options.coin.algorithm].multiplier,
                networkHashRate: rpcResults.getmininginfo.netmhashps
            };
           }else{
           	options.initStats = {
                connections: rpcResults.getinfo.connections,
    ...
    function CheckBlockAccepted(blockHash, callback){
        //setTimeout(function(){
        	console.log('hash do bloco: '+blockHash);
